...
func Serve(fastMode bool, cookieKey string) {
	...
	go func() {
		...
		// 隧道服务（Tailscale、Cloudflared）启动失败不影响核心服务器启动
		go func() {
			defer func() {
				if e := recover(); nil != e {
					logging.LogWarnf("boot tailscale tunnel failed: %v", e)
				}
			}()
			tunnel.InitTailscale()
		}()
		go func() {
			defer func() {
				if e := recover(); nil != e {
					logging.LogWarnf("boot cloudflared tunnel failed: %v", e)
				}
			}()
			tunnel.InitCloudflared()
		}()
	}()
